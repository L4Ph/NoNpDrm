...
static int MakeFakeLicense(char *license_path, SceNpDrmLicense *license_buf) {
  ...
  // Check the existence of fake license file
  SceUID fdc = ksceIoOpen(path, SCE_O_RDONLY, 0);
  if (fdc >= 0) {
  	SceNpDrmLicense clicense;
  	int size = ksceIoRead(fdc, &clicense, sizeof(SceNpDrmLicense));
  	ksceIoClose(fdc);
  	if (size == sizeof(SceNpDrmLicense) && (memcmp(&clicense, &license, sizeof(SceNpDrmLicense)) == 0))
  		return -3;
  }
...
static SceUID _ksceKernelLaunchAppPatched(void *args) {
  ...
  snprintf(license_path, sizeof(license_path)-1, "ux0:license/theme");
  FindLicenses(license_path);
